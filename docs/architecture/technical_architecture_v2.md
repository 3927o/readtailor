...
- `reading_setup_operations`：策略反馈、试读反馈和策略确认的幂等键、request hash、base pointer、
...
大文件上传使用流式 multipart。访谈、策略反馈、试读反馈、策略确认和问 AI 的 POST 请求使用流式
HTTP/SSE 响应；流结束时发送 typed completion event，携带最终保存的业务结果或结果快照。
...
会创建明确副作用的按钮请求携带幂等键。阅读准备命令还校验 request hash；同键同输入返回同一
operation，同键不同输入返回 409。当前不要求所有写接口都实现同等级的 operation 协议。
