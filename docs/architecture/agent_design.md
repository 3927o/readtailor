...
策略反馈、试读反馈和第一次确认都是可恢复的长命令。宿主为每次命令创建或复用
`reading_setup_operation`，使用幂等键、request hash、数据库 lease、attempt fencing 和结果 pointer
管理执行。Agent session 和 HTTP/SSE 连接都不是 operation 的事实来源；连接断开后宿主仍可完成提交，
客户端通过 operation 和精确版本快照恢复。
...
无参数地结束问答阶段。成功后宿主持久化 `completion_started`，此后不再允许提出问题，
但不会立即把 session 标记为 completed，也不会推进业务 workflow。
...
前四个提交工具把校验通过的产物作为访谈 checkpoint 保存；失败或进程重启时从第一个缺失阶段恢复。
`complete_interview_result` 只在产物齐全时组装完整结果，最终由宿主单事务创建画像和策略草稿。
...
首发不保存逐节点处理模式。工具只验证用户可读说明和结构化处理原则；正式 draft 在最终事务中创建。

#### `submit_trial_candidates`

从共享 book profile 候选池中保存恰好三个互不重复的试读候选。候选必须通过 manifest
和 tailoring eligibility 校验后才能写入 checkpoint。

#### `submit_interview_profile`

保存本书读者画像和可选长期画像 patch。长期画像 patch 在最终事务中合并，不需要用户逐次确认。

#### `complete_interview_result`

重新读取四类 durable checkpoint 并完成访谈结果。最终事务成功后应用才进入 `strategy_review`。只有用户明确点击“处理方式没问题，生成试读”，
业务系统才能把当前 draft 标记为 `approved_for_trial`。这不是正式生效，也不创建正式阅读任务。

策略确认阶段的反馈修订仍使用 `save_strategy_draft`。
...
- 用户必须已经明确确认当前策略，operation 的 base draft 必须仍是 current；`approved_for_trial` 标记在
  三个片段全部校验通过的最终事务中写入
- 必须恰好三个互不重叠的片段
...
选择理由写入 segment，并在固定试读槽位中与章节路径一起展示；日志只记录必要标识，不复制完整原文。

工具参数使用增量 parser 读取。只有某个 fragment 对象在原始 JSON 中真实闭合并通过 schema 与业务
范围校验后，宿主才可发送公开 `fragment_selected` 事件；自动补全的 JSON 只能用于字符串展示，不能
制造结构化片段事件。三个 fragment 的 ordinal/tag 固定为 threshold、typical、hardest。

#### 选段后的宿主执行（非 Agent 工具）

Agent 不直接请求或等待试读增强生成。`select_trial_fragments` 成功后，宿主最终事务创建
`trial_revision`、三个 segment 和 generation，切换当前 pointer，随后由 Worker 并行生成。每段状态和
结果独立写回；3/3 ready 后由确定性业务逻辑发布 revision 并推进到 `trial_review`。

每个任务使用与正式章节相同的有限重试策略。任一任务耗尽重试仍失败时，整轮进入
`trial_generation_failed`，不得发布或进入最终确认。原文和已完成段仍可展示。用户技术重试时，宿主
锁定当前 failed revision，按 ordinal 精确复制三个已保存 segment 的 section、segment、range 和 reason，
创建新 revision 和 generation；不得重新调用 `select_trial_fragments`，也不增加 `adjustment_count`。

只有同一 `trial_revision` 下的三个片段都达到 ready，且其
`strategy_draft_version` 仍是当前已批准草稿时，才能发布当前试读版本。`trial_revision` 用于
版本和幂等控制；它与限制用户反馈次数的 `adjustment_count` 是两个不同计数。

“可展示”和“可发布”是两条规则：pending/generating/failed 段始终可展示原文，单段 ready 后可立即
展示该段完整辅助内容；反馈和最终采用仍只对当前 published revision 开放。
...
1. 宿主创建或复用绑定当前 trial revision 的 strategy revision operation，旧 trial 保持 current。
2. 反馈作为新 user message 写入同一个逻辑业务会话，Agent 修订文字处理方式和结构化策略草稿。
3. HTTP/SSE 断开或 lease 接管时 operation 进入恢复流程，不能 supersede 旧 trial 或重复计数。
4. 只有最终事务成功后，才原子 supersede 旧 trial、保存新草稿、增加调整次数并返回
   `strategy_review`。
5. operation 明确失败时旧 trial 和反馈仍可恢复；用户重新确认新策略后才重新选择片段。
...
第一次确认 `approveDraftForTrial` 发生在 `strategy_review`：用户批准当前 draft 仅用于生成试读片段。

第二次确认 `confirmStrategyAndStartReading` 发生在 `trial_review`。只有用户点击
“采用这个处理方式并开始阅读”后，业务系统才能：

- 校验用户提交的 `trial_revision` 是当前可用版本，且其中的 `strategy_draft_version` 是当前
  已批准草稿
- 校验三个试读片段均已发布且为 ready
- 将试读对应的 draft 标记为 confirmed
- 创建生效的 `strategy_version`
- 把用户书籍状态改为 `active_reading`
- 创建第一个正式阅读节点及后续三个节点的生成任务

最终采用使用业务状态幂等，不要求幂等 key。重复或并发请求必须返回同一个正式
`strategy_version`，不能创建第二组正式节点生成记录。
...
- 创建和发布试读使用 `trial_revision + strategy_draft_version`
- 生成节点使用 `generation_job_id`
- 规范化完成使用 `normalization_job_id + script_hash + output_hash`

`approveDraftForTrial` 属于 reading setup operation，使用稳定幂等 key 和 request hash；
`confirmStrategyAndStartReading` 不使用幂等 key，而以当前 trial/draft pointer 和 `active_reading` 状态幂等。

重复工具调用不得生成多个生效版本。

阅读准备命令由宿主把规范化输入计算为 request hash：相同 user book、幂等键和 hash 返回同一
operation，completed 直接按结果 pointer 重放；相同键但 hash 不同返回冲突。网络重连、重复传输
和 resume 必须复用原键，用户在 operation 明确 failed 后重新提交才创建新键。
...
- 工具错误
- 最终完成或停止原因
- reading setup `operation_id`、kind/source、attempt、lease reclaim/lost 和结果 pointer
- 首个策略增量、首个 fragment、单段 ready、3/3 published 和 disconnect/resume 耗时
...
- [ ] 新反馈成功提交前旧 `trial_revision` 保持 current；失败时不丢旧样章或重复计数
- [ ] 单段 ready 后可展示该段结果，但 3/3 published 前不得反馈或最终采用
- [ ] 任一试读任务重试耗尽后不得发布或进入最终确认，技术 retry 精确复制原三段且不重新选段
