...
### 9.3 正式采用（`confirmStrategyAndStartReading`）

用户查看三个片段并最终确认时，API 在一个事务中：

- 校验三个片段均已发布且为 ready；查看记录不作为采用门禁。
- 校验 trial revision 和 strategy draft 均为当前版本。
- 创建不可变正式 strategy version。
- 把用户书籍状态改为 `active_reading`。
- 创建第一个正式节点及后续三个可裁读节点的生成 job。

最终采用不要求 idempotency key，而是使用业务状态幂等：若用户书籍已经进入 `active_reading`，
重复请求直接返回当前正式 strategy version；并发请求只能创建一个正式版本和一组正式生成 job。
...
当前初始流程的最终采用是例外：`confirmStrategyAndStartReading` 由请求中的 current trial/draft pointer
和数据库事务做状态幂等，不发送 `Idempotency-Key`。
