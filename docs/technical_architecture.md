...
1. 将草稿标记为 approved-for-trial。
2. 创建新的 `trial_revision`。
3. 保存恰好三个互不重叠、位于可裁读节点内的连续范围。
4. 创建三个生成 job。

三个任务可以并行执行，但只有全部成功且版本门禁一致时，事务才把 revision 改为 `ready`。
任一任务三次失败后整轮 `failed`，不发布部分结果。用户反馈会立即 supersede 整个旧 revision
和未完成 job。
...
- 大文件上传使用流式 multipart。
- 访谈和问 AI 的 POST 请求都使用流式 HTTP 响应，用户可见文本增量由 API 直接写回浏览器。
...
业务事件先持久化到 `business_events`，SSE 按用户权限读取并映射。事件包含递增 id、类型、资源
id、资源 version 和产品化 payload。客户端使用 `Last-Event-ID` 断线续传；无法补齐时重新查询
资源快照。SSE 不是业务事实来源，也不传输模型思维过程或内部工具日志。

交互式 Agent 的 token/文本增量不写入 `business_events`，而是由处理该请求的 API 实例直接
返回。完整 assistant message 和结构化产物只在通过发布门禁后写入 PostgreSQL。连接中断时当前
run 可以取消并标记失败；用户重试同一条已保存消息，不创建重复业务版本。
...
同一逻辑会话同时只能有一个 active turn。API 使用 PostgreSQL advisory lock 或带过期时间的
conversation lease，并结合 `expected_version` 防止多标签页并发推进。请求开始和结束记录在
`ai_runs`；API 进程崩溃后，维护任务把超时的 `running` run 标记为 `abandoned`，用户可以重试。
