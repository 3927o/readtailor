...
本文记录 handoff 进入新项目后的实施取舍。它不改变 `product_prd.md` 中的用户可见行为、
`reading_contract.md` 中的阅读位置契约或 `normalized_book_spec.md` 中的规范化要求。

## 1. 文档优先级

出现冲突时按以下顺序处理：

1. `product_prd.md`：用户可见行为和验收规则。
2. `reading_contract.md`：阅读节点、block、range、进度和统计。
3. `normalized_book_spec.md`：规范化书籍产物。
4. `agent_design.md`：Agent 职责、工具和权限。
5. `technical_architecture_v2.md`：当前实现方案。
6. `product_mvp_plan.md`：产品方向和背景说明。
7. `technical_architecture.md`：未来加固参考。
