...
> 本文件最初基于本地 `main` 的 `06fb00e` 状态整理；阶段 A-F 已于 2026-07-15 落地。
> 第 1 节保留实施前基线用于解释改造动机，当前行为以第 2-9 节、PRD 和技术架构为准。
...
- 实施前策略确认请求会等待 `select_trial_fragments` 完整返回三个片段。
...
PRD、架构文档和 `core_flow_refactor.md` 已同步为“允许部分展示、禁止部分发布和采用”。
...
11. stream 和 operation resume 必须调用同一 operation service，不能绕过 pending/running operation
    约束另起 Agent 调用。

客户端在一次用户命令开始时生成 `idempotencyKey`，并持续复用到该命令 completed，或 failed 后用户明确重新提交。网络重连、重复传输和 resume 不能临时生成新 key；收到 `operationId` 后优先使用 operation resume，不再重新提交原命令。本方案不提供运行中取消 operation 的能力。
...
实施时将旧 `createInterviewStreamParser` 替换为按工具分发的 `createReadingSetupStreamParser`。
...
feedback/approve 只保留 stream 写端点；恢复统一通过 operation resume 和精确 snapshot GET，不能启动
第二套同步 Agent 流程。
...
- SSE 与 resume 入口只能调用以上 service，差异仅限于是否转发中间事件。
...
现有无版本 GET 保留为 current alias。Web 的版本化 query key 使用 by-id endpoint；completed operation
重放返回历史 snapshot 时只能写该历史版本 key，不能修改 current pointer 或覆盖新版本缓存。
...
- SSE 和 resume 入口共享同一 operation、lease、attempt 和最终事务。
...
### 阶段 A：契约、parser 与 operation 基础设施（已完成）
...
6. 修改 approve contract，使所有 reading setup 写命令都显式接收 `idempotencyKey`；Web API helper 改为接收命令创建时生成的 key，不能在每次 `fetch` 内生成。
7. 增加 by-id strategy/trial snapshot service 与端点，并让 Web detail mapper 保留 current pointers、建立版本化 query key 基础设施。
8. 让所有阅读准备写命令统一进入 operation service，页面展示暂时保持原状。

验收：approve/feedback 客户端在一次用户命令内持有稳定 key，内部统一使用 operation；现有 Web 流程
无回归，新 parser/operation 测试通过。
...
1. 删除不再使用的非流式 feedback/approve 写端点，Web 只使用 stream + operation resume。
2. 删除旧通用访谈完成事件，清理“整轮完成后才展示”的 UI 和文档口径。
3. 更新 PRD、agent design、technical architecture 和 core flow refactor。
4. 在 `technical_architecture_v2.md` 补齐可观测性指标和阅读准备排障步骤。
...
- strategy feedback、trial feedback 和 approve 的 stream 请求都要求 `idempotencyKey`；by-id snapshot 与 operation detail 参数只接受合法 UUID。
...
- 同一 stream 命令使用同一 key 重放时返回同一 operation，不会并行调用 agent。
...
具体排障顺序见 `../architecture/technical_architecture_v2.md` §16.1。
...
控制：operation 只覆盖策略修订和试读切片，不替代业务表；不存字符事件；SSE 与 resume 共用 command service；claim、续租、attempt 和 fencing 集中在单一模块。
