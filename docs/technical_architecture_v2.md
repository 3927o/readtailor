...
- workflow lease、fencing token 和分布式锁体系
- 为多标签页、多实例和极端竞态设计的复杂并发协议
...
页面刷新后从 API 重新读取当前状态。前端不自行推断或推进服务端流程。
...
当前依赖 BullMQ 的 stalled 检测和重试机制，不额外实现数据库 lease、fencing token 或完整的崩溃
接管协议。出现数据库 job 长时间停留在 running 的情况时，由简单维护任务标记失败并允许用户重试。
...
当前不实现分布式 session cache、conversation lease 或跨实例 active turn 接管。提交回复时使用简单的
`conversation_version` 条件更新；发生冲突时当前请求失败，用户重新提交。

### 10.3 试读

用户第一次确认策略后：

1. 当前策略草稿标记为 approved-for-trial。
2. 创建一个 `trial_revision`。
3. 保存恰好三个互不重叠、位于可裁读节点内的连续范围。
4. 创建三个后台生成任务。
5. 三个结果全部完成后进入试读页面。

任一片段失败时整轮显示失败，不发布不完整试读。用户反馈后创建新的 revision，页面只展示当前
revision。
...
重复点击最终确认通过 `Idempotency-Key` 去重，避免创建多个正式策略。
...
用户明确确认后创建新策略，并为后续未读节点创建新任务；当前节点和已读节点继续使用旧结果。

当前只保证主流程和单次采用事务正确，不处理同时打开多个页面修改策略的全部边缘情况，也不对正在
运行的所有旧任务执行复杂版本门禁；读取结果时只选择当前策略对应的内容。
...
大文件上传使用流式 multipart。访谈和问 AI 的 POST 请求使用流式 HTTP 响应，文本增量由处理请求的
API 实例直接返回；流结束时发送 typed completion event，携带最终保存的业务结果。
...
SSE 不发送模型思维过程、原始工具参数或内部日志。连接中断时保留已保存的用户输入，用户可以重试
当前 turn。

会创建明确副作用的按钮请求携带 `Idempotency-Key`，服务端对关键操作做简单去重。当前不要求所有
写接口都实现完整 optimistic concurrency control。
