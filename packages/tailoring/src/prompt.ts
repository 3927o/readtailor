// Builds the versioned prompt shared by trial and formal tailoring generation.
...
export function buildTailoringPrompt(input: TailoringGenerationInput): string {
  ...
  const payload = {
    ...
    blocks: input.source.blocks.map(({ blockIndex, kind, text }) => ({ blockIndex, kind, text })),
    // 前后文暂不进入模型输入，保留源数据字段便于后续重新评估。
    // adjacentContext: {
