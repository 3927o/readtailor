...
const IDLE_STREAM: StreamState = { active: false, concluding: false, ack: '', prompt: '', options: [], sufficiency: null };
...
export function InterviewPage() {
  ...
  if (interview.isPending) {
    return <WorkflowPage book={book} kicker="A CONVERSATION · 本书访谈" title="先聊几句"><WorkflowMessage title="正在恢复访谈">答案和当前问题正在从服务端读取。</WorkflowMessage></WorkflowPage>;
  }
  if (interview.isError) {
    return <WorkflowPage book={book} kicker="A CONVERSATION · 本书访谈" title="先聊几句"><WorkflowMessage title="访谈暂时没有打开" action={<button className="button button-ghost" type="button" onClick={() => void interview.refetch()}>重新读取</button>}>{interview.error.message}</WorkflowMessage></WorkflowPage>;
  }
  ...
  return (
    <WorkflowPage book={book} kicker="A CONVERSATION · 本书访谈" title="先聊几句">
        ...
        {snapshot.history.length ? (
          <div className="interview-history" aria-label="之前的回答">
            {snapshot.history.map((item, index) => (
              <div className="interview-hist" key={`${index}:${item.answer}`}>
                <p>{item.question}</p>
