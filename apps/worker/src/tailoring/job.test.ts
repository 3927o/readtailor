/** Verifies formal-generation staleness, scope, and attempt fencing. */
...
describe('formal-only generation boundary', () => {
  it('rejects a database trial generation before attempting finalization', async () => {
    const generation = {
      generationScope: 'trial',
    } as Parameters<typeof finalizeContentGeneration>[0]['generation'];
    await expect(finalizeContentGeneration({
      db: {} as Parameters<typeof finalizeContentGeneration>[0]['db'],
      generation,
      claimedAttempt: 1,
      result: { guide: null, annotations: [], afterReading: null },
    })).rejects.toThrow('only supports formal generations');
  });
});
